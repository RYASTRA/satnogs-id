...
if os.path.exists(_cache):
    ...
    cand_obs = {
        n: getj(
            f"https://network.satnogs.org/api/observations/?norad_cat_id={n}&format=json"
        )
        for n in soup
    }
...
existing = {
    os.path.basename(p).split("_")[0][3:] for p in _glob.glob(SC + "/eval/*.h5")
}
for norad, name in GEOSCANS.items():
    ...
    for o in cands:
        ...
        if str(oid) in existing or os.path.exists(fn):
            ...
            continue
        rows = getj(
            f"https://db.satnogs.org/api/artifacts/?network_obs_id={oid}&format=json",
            auth=True,
        )
...
for oid, norad in downloaded:
    tdate = datetime.fromisoformat(
        getj(f"https://network.satnogs.org/api/observations/?id={oid}&format=json")[0][
            "start"
        ][:10]
    )
    lines = []
    for n in soup:
        ...
        if best:
            lines += [
                (best.get("tle0") or "0 OBJECT").strip(),
                best["tle1"],
                best["tle2"],
            ]
