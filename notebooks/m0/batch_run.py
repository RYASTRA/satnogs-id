...
def make_dat(h5path, siteid, datpath):
    ...
    win = max(
        3, int(4000 / dfbin)
    )  # +-4 kHz search window around the (near-vertical) carrier
    ...
    for _ in range(
        3
    ):  # iterative MAD outlier rejection (vertical line -> ~constant offset)
        med = np.median(pts[:, 1])
        mad = np.median(np.abs(pts[:, 1] - med)) + 1e-9
    ...
    for relt_i, foff in pts:
        ...
        rows.append(
            ((dt - EPOCH).total_seconds() / 86400.0, f0 + float(foff) - f0 * rr / C)
        )
    rows.sort()
    with open(datpath, "w") as g:
        for mj, fr in rows:
            g.write(f"{mj:.6f}\t{fr:.2f}\t1.0\t{siteid}\n")
    with open("/opt/strf/data/sites.txt", "a") as g:
        g.write(
            f"{siteid} GS {loc['latitude']:.4f} {loc['longitude']:.4f} {int(loc['altitude'])} obs\n"
        )
    return len(rows)
